import React, { useState } from 'react'
import { AiOutlineMenu, AiOutlineClose } from 'react-icons/ai'
import { motion } from 'framer-motion'
const navLinks = [
  { title: 'About', path: '#about' },
  { title: 'Portfolio', path: '#portfolio' }
]

function Navigation () {
  const [nav, setNav] = useState(false)

  const toggleNav = () => {
    setNav(!nav)
  }

  const closeNav = () => {
    setNav(false)
  }

  const navVariants = {
    open: {
      y: 0,
      transition: {
        stiffness: 20,
        damping: 15
      }
    },
    closed: {
      y: '-100%',
      transition: {
        stiffness: 20,
        damping: 15
      }
    }
  }

  return (
    <div className='text-white/70 pt-6'>
      <div className='hidden md:flex item-center px-4 py-2 mx-auto max-w-[400px]'>
        <ul className='flex flex-row p-4 space-x-8'>
          {navLinks.map((item, index) => (
            <li key={index}>
              <a href={item.path}>{item.title}</a>
              {/* <Link to={item.path}>{item.title}</Link> */}
            </li>
          ))}
          <li>
            <a href='#contact' className='group'>
              <h1 className='cursor-pointer font-bold text-lg'>Contact Me</h1>
              <div className='relative'>
                <div className='absolute w-2/3 h-1 transition-all duration-300 ease-out bg-orange-400 rounded-full group-hover:w-full'></div>
                <div className='absolute w-1/3 mt-1 h-1 transition-all duration-300 ease-out bg-orange-800 rounded-full group-hover:w-full'></div>
              </div>
            </a>
          </li>
        </ul>
      </div>

      <div
        onClick={toggleNav}
        className='md:hidden absolute top-5 right-5 border rounded text-white/70 border-white/70 p-2 z-50'
      >
        {nav ? <AiOutlineClose size={20} /> : <AiOutlineMenu size={20} />}
      </div>

      <motion.div
        initial={false}
        animate={nav ? 'open' : 'closed'}
        variants={navVariants}
        className='fixed left-0 top-0 w-full z-40'
      >
        <ul className='text-2xl font-semibold my-5 text-center space-y-2'>
          {navLinks.map((item, index) => (
            <li key={index}>
                            <a href={item.path} onClick={closeNav} b>{item.title}</a>

              {/* <Link to={item.path} onClick={closeNav}>
                {item.title}
              </Link> */}
            </li>
          ))}
        </ul>
      </motion.div>
    </div>
  )
}

export default Navigation
